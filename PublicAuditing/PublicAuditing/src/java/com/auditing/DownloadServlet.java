...
public class DownloadServlet extends HttpServlet {
 ...
 private static Key generateKey(String productkey) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		PreparedStatement ps=con.prepareStatement("insert into detail(username,groupname,filename,date,time) values('"+username+"','"+group+"','"+filename+"','"+sqlDate+"','"+sqlTime+"')");
                
                 int i=ps.executeUpdate();
                        if(i>0)
                        {
            System.out.println("33333");  
            String TempDownloadDirectory="D:/temp15/";
